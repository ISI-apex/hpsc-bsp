...
function setup_rtems_env()
{
    ...
    local RTEMS_PREFIX="${BSP_DIR}/${WORKING_DIR#${BSP_DIR}}/env/RTEMS-5-TRCH"
    if [ ! -d "$RSB_PREFIX" ]; then
        echo "Recipe not built: sdk/rtems-source-builder"
        return 1
    fi
    if [ ! -d "$RTEMS_PREFIX" ]; then
        echo "Recipe not built: ssw/trch/rtems"
        return 1
    fi
    # export environment variables
    export PATH=${RSB_PREFIX}/bin:$PATH
    export RTEMS_MAKEFILE_PATH="${RTEMS_PREFIX}/arm-rtems5/hpsc_trch_qemu"
}
