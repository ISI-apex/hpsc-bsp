...
#
# 2.  Build the hpsc-baremetal repository (not part of Yocto build) which
#     will create the baremetal firmware files trch.elf and rtps.elf.
#     Once built, adjust the BAREMETAL_PATH variable below.
#
# 3.  Update the MY_IP_ADDR variable below.

# The following two variables need to be updated:
BAREMETAL_PATH=/home/kdatta/Documents/isi_git_repos/hpsc-baremetal
...
# Output files from the hpsc-baremetal repo build
TRCH_FILE=$BAREMETAL_PATH/trch/bld/trch.elf
RTPS_FILE=$BAREMETAL_PATH/rtps/bld/rtps.elf
